...
export class ShaderUtils {
    ...
    private static readonly defaultVertexShaderSource: string = `
        precision highp float;
        attribute vec3 a_position;
        uniform mat4 u_matrixTranslation;
        uniform mat4 u_matrixProjection;
        uniform mat4 u_matrixRotation;  
        uniform mat4 u_matrixScale;    
        uniform mat4 u_matrixView;   
        void main() {
            mat4 TRS = u_matrixTranslation * u_matrixRotation * u_matrixScale;
            vec4 model = TRS * vec4(a_position, 1.0);
            vec4 MVP = u_matrixProjection * u_matrixView * model;
            gl_Position = MVP;
        }
    `;

    private static readonly defaultFragmentShaderSource: string = `
        precision highp float;
    ...
    public static createVertexShader(gl: WebGLRenderingContext, source: string): WebGLShader | null {
        ...
        if (!vertShader) {
            ...
            vertShader = ShaderUtils.createShader(gl, gl.VERTEX_SHADER, ShaderUtils.defaultVertexShaderSource);
        }
    ...
    public static createFragmentShader(gl: WebGLRenderingContext, source: string): WebGLShader | null {
        ...
        if (!fragShader) {
            ...
            fragShader = ShaderUtils.createShader(gl, gl.FRAGMENT_SHADER, ShaderUtils.defaultFragmentShaderSource);
        }
